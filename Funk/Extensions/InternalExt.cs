...
internal static class InternalExt
{
    ...
    private static MemberExpression GetMemberExpression<T>(this Expression<Func<T, object>> expression) =>
        expression is null ? throw new ArgumentNullException(nameof(expression)) :
        Unwrap(expression.Body) as MemberExpression ?? throw new ArgumentException("The expression does not indicate a valid property or a field.");
